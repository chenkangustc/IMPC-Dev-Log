# IMPC-Dev-Log
## 20180418 计算结果测试（开发日志转到本地）
* 下午小目标：添加堆芯、换热器、管道的导热项；
* 测试大功率大流速下的计算结果
* 研究导热的影响
* 在冷却剂求解中加入导热项；
* 稳态计算结果分析；
* 合并堆芯和回路的输入卡
* 燃料棒固体的热物性应该从daisy继承；（先单独输入，方便单独建模，单独输入可采用同样的物性公式）
## 20180416 给出控制方程，写新的堆芯程序，修改原来的堆芯程序；

## 20180413 今日事今日毕，完成后处理模块和堆芯求解部分；先写堆芯求解部分；
* 查看稳态分布；
* 增加pow的计算和输出。
* 大胆尝试（快速迭代优化），小心求证（做好版本管理）
* loop数据块中声明loop，增加输入卡中的loop:flowrate,输入，其他所有卡中的Q都由init获得，瞬态的由cal获得；
* 回路流量设置为共享数据块；在初始化的时候各部件流量应等于回路流量。
* 各个类只计算热工；
* 水力学计算为整个回路公有；先实现后优化，先实现再说。
## 20180412
* 热工模块测试；
* 程序优化：合并堆芯和回路输入卡；
* 程序优化：输出功率之和；
## 20180404
* 面试准备：熟悉讲稿；常见问答；实：算法工程师岗位职责；Andger ML；
## 20180403
* 完成何明涛工作调研；华电、西交、哈工程、印度的验证手段调研；15:22-17:30
* 17:30-18:00准备组会PPT；熟悉面试讲稿；晚上准备面试相关事宜：查看相关专利，多了解算法工程师岗位职责；
* 重要的事先做；
* 继续调研完各家单位的验证手段并模仿；调研调研；
* 从各个模型出发回路流量模型、换热器模型、管道模型、堆芯模型依次验证；
## 20180402
* ks的问题；调研需要计算的工况；确定程序验证方案；
* 自我介绍：0.5，问答1.0,11:00
* 毕业论文计划表；4月计划；清明计划；本周计划；0.5h
## 20180331
* 写自我介绍讲稿；
* 本机停止计算，继续校验稳态的计算结果和给出合适的后处理；
* 大小服务器各有一个算例在测试瞬态计算是否能跑通；
* 在另外两台机器上安装调试编译环境，用以测试程序；
* 在本机上继续验证计算结果，调整后处理，优化代码；
## 20180324
